...
if [[ -z "$GITHUB_ACTIONS" ]] ;then
    ...
    target_stage=s3://sfc-eng-jenkins/repository/hibernate/$BRANCH/${GIT_COMMIT}
    echo "[INFO] Uploading jar to $target_stage/"
    aws s3 cp --only-show-errors "$HIBERNATE_ROOT"/lib/ "$target_stage/" --recursive --exclude "*" --include "*.jar"
    aws s3 cp --only-show-errors "$HIBERNATE_ROOT/FIPS/lib $target_stage/" --recursive --exclude "*" --include "*.jar"
...
COMMIT_FILE_CONTENTS
    ...
    cp -p "$HIBERNATE_ROOT/lib/*.jar" "$WORKSPACE/artifacts"
    cp -p "$HIBERNATE_ROOT/FIPS/lib/*.jar" "$WORKSPACE/artifacts"
