...
public class SnowflakeDialect extends Dialect {
    ...
    final static String ALLOW_UNRECOMMENDED_JDBC_DRIVER_VERSION = CONFIGURATION_PROPERTY_PREFIX + "hibernate.dialect.snowflake.allow_unrecommended_jdbc_driver";
    ...
    public SnowflakeDialect(DialectResolutionInfo info) {
        ...
        boolean allowUnrecommendedJdbcVersion = Boolean.parseBoolean((String) configurationValues.getOrDefault(ALLOW_UNRECOMMENDED_JDBC_DRIVER_VERSION, "false"));
        warnOnUnsupportedDriverVersion(allowUnrecommendedJdbcVersion);
    }

    private void warnOnUnsupportedDriverVersion(boolean allowUnrecommendedJdbcDriverVersion) {
        ...
        try {
            ...
            log.info("Dialect property {} has value {}", ALLOW_UNRECOMMENDED_JDBC_DRIVER_VERSION, allowUnrecommendedJdbcDriverVersion);
            log.info("JDBC Driver version {}", driverVersion);
            if (MINIMAL_DRIVER_VERSION.compareTo(driverVersion) > 0) {
                ...
                if (allowUnrecommendedJdbcDriverVersion) {
    ...
    private void warnOnUsingNonHybridTables() {
        ...
        if (tableType != TableType.HYBRID) {
            log.warn("Be aware, that not hybrid tables don't enforce constraints (see: https://docs.snowflake.com/en/sql-reference/constraints-overview). It can also reduce performance as hybrid tables are preferred for OLTP.");
        }
    }

    private static void warnOnPossibleSensitiveDataLoggingEnabled(Map<String, Object> configurationValues) {
        // security risk mitigation
        boolean developmentMode = Boolean.parseBoolean(configurationValues.getOrDefault(CONFIGURATION_PROPERTY_DEVELOPMENT_MODE, "false").toString());
        warnOnPossibleSensitiveDataLogging(developmentMode, LoggerFactory.getLogger(HIBERNATE_PARAMETER_BINDING_LOGGER),
                "Statement parameter bindings logging is enabled - it's recommended to turn it off on production environment");
        warnOnPossibleSensitiveDataLogging(developmentMode, LoggerFactory.getLogger(HIBERNATE_EXTRACTED_VALUES_LOGGER),
                "Extracted in select data logging is enabled - it's recommended to turn it off on production environment");
    }

    private static void warnOnPossibleSensitiveDataLogging(boolean developmentMode, Logger sensitiveDataLogger, String logMessage) {
    ...
    public String getForUpdateString() {
        ...
        // we should revisit lock strategies and/or prohibit using table id generator for standard tables
    ...
    protected void registerDefaultKeywords() {
        ...
        // some Snowflake keywords are provided by the driver: https://github.com/snowflakedb/snowflake-jdbc/blob/fa658a430cb9b810c791753ad1f0afb29c5528f3/src/main/java/net/snowflake/client/jdbc/SnowflakeDatabaseMetaData.java#L96
        // but some are missing - there is an OSS issue to fix it https://github.com/snowflakedb/snowflake-jdbc/issues/1630
        // state based on driver version 3.15.0
        // keywords are automatically applied when hibernate property hibernate.auto_quote_keyword is set to true
    ...
    public int getDefaultTimestampPrecision() {
        // Default timestamp precision as in Snowflake documentation https://docs.snowflake.com/en/sql-reference/data-types-datetime#timestamp
        return 9;
    }
    ...
    protected String columnType(int sqlTypeCode) {
        ...
        // CLOB and BLOB are supported by different types https://docs.snowflake.com/en/sql-reference/data-types-unsupported
        switch (sqlTypeCode) {
    ...
    public TimeZoneSupport getTimeZoneSupport() {
        ...
        // It seems that the best way to keep date with timezone from Hibernate is to have a split table with timezone
